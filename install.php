...
if ($configExists) {
    // Suppress errors from including config
    @include($configFile);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($step === 'database') {
        ...
        if (empty($xaiKey)) {
            ...
            $configContent .= "header('Access-Control-Allow-Methods: GET, POST, OPTIONS');\n";
            $configContent .= "header('Access-Control-Allow-Headers: Content-Type, Authorization');\n";
            $configContent .= "if (\$_SERVER['REQUEST_METHOD'] === 'OPTIONS') { http_response_code(200); exit; }\n\n";
            $configContent .= "// Database\n";
            $configContent .= "define('DB_HOST', " . var_export($db['dbHost'] ?? 'localhost', true) . ");\n";
            $configContent .= "define('DB_NAME', " . var_export($db['dbName'] ?? '', true) . ");\n";
            $configContent .= "define('DB_USER', " . var_export($db['dbUser'] ?? 'root', true) . ");\n";
            $configContent .= "define('DB_PASS', " . var_export($db['dbPass'] ?? '', true) . ");\n\n";
            $configContent .= "// xAI API\n";
            $configContent .= "define('XAI_API_KEY', " . var_export($xaiKey, true) . ");\n";
            $configContent .= "define('XAI_BASE_URL', 'https://api.x.ai/v1');\n\n";
            $configContent .= "// Firebase\n";
            $configContent .= "define('FIREBASE_PROJECT_ID', " . var_export($fbProjectId, true) . ");\n\n";
            $configContent .= "// bKash\n";
            $configContent .= "define('BKASH_APP_KEY', " . var_export($bkashAppKey, true) . ");\n";
            $configContent .= "define('BKASH_APP_SECRET', " . var_export($bkashAppSecret, true) . ");\n";
            $configContent .= "define('BKASH_USERNAME', " . var_export($bkashUsername, true) . ");\n";
            $configContent .= "define('BKASH_PASSWORD', " . var_export($bkashPassword, true) . ");\n";
            $configContent .= "define('BKASH_BASE_URL', " . var_export($bkashBaseUrl, true) . ");\n\n";
            $configContent .= "// Site\n";
            $configContent .= "define('SITE_URL', " . var_export($siteUrl ?: 'http://localhost/groksubscription', true) . ");\n";
